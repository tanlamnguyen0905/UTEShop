...
public class ProductDaoImpl implements ProductDao {
    ...
    public Product findById(int id) {
        EntityManager em = JPAConfig.getEntityManager();
        try {
            // Use em.find to avoid multiple JOIN FETCH issues; collections will be
            // lazy-loaded
    ...
    public List<Product> findByName(String name) {
        ...
        try {
            TypedQuery<Product> query = em
                    .createQuery(BASE_QUERY_SIMPLE + "WHERE LOWER(p.productName) LIKE LOWER(:name)", Product.class);
    ...
    public List<Product> findByCategoryIdinPage(int categoryId, int page, int pageSize) {
        ...
        try {
            TypedQuery<Product> query = em.createQuery(
                    BASE_FETCH + "WHERE p.category.categoryID = :id ORDER BY p.productID DESC", Product.class);
            query.setParameter("id", (long) categoryId);
    ...
    public List<Product> findBestSeller(int limit) {
        ...
        try {
            TypedQuery<Product> query = em
                    .createQuery(BASE_FETCH + "left joint p.orderDetails ORDER BY p.soldCount DESC", Product.class);
    ...
    public List<Product> findByPriceRange(double minPrice, double maxPrice) {
        EntityManager em = JPAConfig.getEntityManager();
        try {
            TypedQuery<Product> query = em.createQuery(BASE_FETCH + "WHERE p.unitPrice BETWEEN :minPrice AND :maxPrice",
                    Product.class);
    ...
    public List<Product> findByPriceRangeinPage(double minPrice, double maxPrice, int page, int pageSize) {
        ...
        try {
            TypedQuery<Product> query = em.createQuery(
                    BASE_FETCH + "WHERE p.unitPrice BETWEEN :minPrice AND :maxPrice ORDER BY p.productID DESC",
                    Product.class);
            query.setParameter("minPrice", minPrice);
            query.setParameter("maxPrice", maxPrice);
            query.setFirstResult((page - 1) * pageSize);
    ...
    public List<Product> findProductsByFilter(ProductFilter filter, int page, int pageSize) {
        StringBuilder jpql = new StringBuilder(
                BASE_FETCH + "LEFT JOIN p.orderDetails od LEFT JOIN p.favorites f LEFT JOIN p.banners b WHERE 1=1");
        ...
        switch (sortBy) {
            case "1":
                ...
                break;
            case "2":
                jpql.append(" LEFT JOIN p.reviews r GROUP BY p ORDER BY AVG(r.rating) DESC, COUNT(r) DESC");
                break;
            case "3":
                jpql.append(" LEFT JOIN p.favorites f GROUP BY p ORDER BY COUNT(f) DESC");
                break;
            case "4":
    ...
    public int countProductsByFilter(ProductFilter filter) {
        StringBuilder jpql = new StringBuilder(
                "SELECT COUNT(DISTINCT p) FROM Product p LEFT JOIN p.orderDetails od LEFT JOIN p.favorites f LEFT JOIN p.banners b WHERE 1=1");
    ...
    public List<Product> findByNamePaginated(String name, int firstResult, int maxResults) {
        ...
        try {
            TypedQuery<Product> query = em.createQuery(
                    BASE_QUERY_SIMPLE + "WHERE LOWER(p.productName) LIKE LOWER(:name) ORDER BY p.productID DESC",
                    Product.class);
            query.setParameter("name", "%" + name + "%");
